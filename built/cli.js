...
const getInputFilePath = () => {
    ...
    if (!args?.length) {
        throw new Error('No path to the design file provided.');
    }
    const path = resolve(args[0]);
    if (extname(path) !== SUPPORTED_INPUT_FILE) {
        throw new Error('The design file is not a JavaScript file. Retry using a file with extension ending in .js');
    }
    if (!existsSync(path)) {
